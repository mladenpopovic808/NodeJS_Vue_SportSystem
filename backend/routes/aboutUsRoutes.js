...
const { sequelize, AboutUs,User } = require('../models');
const jwt = require('jsonwebtoken');
require('dotenv').config();
const { idSchema,userSchema ,aboutUsScheme} = require('../joiValidations');
...
route.put('/updateAboutUs', (req, res) => {

    User.findOne({ where: { id: req.user.userId } })
        .then( usr => {
             if (usr.admin || usr.moderator) {
                ///uvek ce biti id=1
                AboutUs.findOne({ where: { id: 1 } })
                .then( aboutUs => {
                    if(aboutUs){
                          const result = aboutUsScheme.validate(req.body);
                          if(result.error){
                                console.log(result.error.message);
                                res.status(422).json({ msg: 'Uneli ste nevalidne podatke! ' + result.error.message });
                            } else {
                                AboutUs.update(req.body, { where: { id: 1 } })
                                    .then( () => res.json({ msg: 'Uspesno ste izmenili aboutUs sekciju!' }) )
